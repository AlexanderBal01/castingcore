...
import { NavigationMenu, Separator, Dialog, VisuallyHidden } from "radix-ui";
...
const Navbar = () => {
  ...
  return (
    <>
      {/* Desktop Navigation */}
      <NavigationMenu.Root className="center relative z-20 hidden h-10 w-screen justify-between px-20 lg:flex">
        <NavigationMenu.Sub className="h-10 list-none content-center">
          <Link href={"/"} className="flex gap-x-2.5">
            <Image
              src={"/logo.svg"}
      ...
      <div className="relative z-20 flex items-center justify-between px-5 py-3 lg:hidden">
        ...
        <Dialog.Root open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
          <Dialog.Trigger asChild>
            <button
              className="rounded bg-white/25 p-2 text-white backdrop-blur-md"
            ...
            <Dialog.Content className="data-[state=open]:animate-slide-in-from-right data-[state=closed]:animate-slide-out-to-right fixed top-0 right-0 z-50 flex h-full w-full flex-col bg-white/25 shadow-lg backdrop-blur-md">
              ...
              <div className="flex-1 overflow-y-auto px-5">
                <div className="border-b border-white py-5">
                  <Link href={"/models"} className="block text-white">
                    Models
                  </Link>
                </div>
                <div className="border-b border-white py-5">
                  <Link href={"/jobs"} className="block text-white">
                    Jobs
                  </Link>
                </div>
                <div className="border-b border-white py-5">
                  <Link href={"/about-us"} className="block text-white">
                    About us
                  </Link>
                </div>
                <div className="border-b border-white py-5">
              ...
              <div className="space-y-5 p-5">
                <Link
                  href="/sign-up"
                  className="block w-full rounded bg-white/30 px-4 py-3 text-center font-medium text-white backdrop-blur-md transition-colors"
                ...
                <Link
                  href="/sign-in"
                  className="block w-full rounded border border-white/30 px-4 py-3 text-center font-medium text-white transition-colors"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Sign in
                </Link>
...
type NavbarListItemProps = Omit<React.ComponentPropsWithoutRef<"a">, "href"> & {
  title: string;
  href: string;
};
